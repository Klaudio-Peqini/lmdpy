...
(function () {
  ...
  class LangevinApp {
    ...
    _buildUI() {
      ...
      createElement("h2", { text: "Langevin Lab" }, root);

      const controls = createElement("div", { class: "lmdpy-controls" }, root);

      // --- Potential selector + tooltip -----------------------------------
      ...
      // tooltip text for each potential
      const potHints = {
        pdg:
          "P–DG coherent domain potential:\n" +
          "U(r) = ½ k · |r|²  −  A_cd · exp( − |r|² / (2 ξ²) ).\n" +
          "Harmonic confinement + coherent water mediated attraction.",

        harmonic:
          "Harmonic trap:\n" +
          "U(r) = ½ k · |r|².\n" +
          "Models optical traps, elastic confinement, baseline reference.",

        double_well:
          "Double well (x direction):\n" +
          "U(x,y,z) = a x⁴ − b x² + ½ k (y² + z²).\n" +
          "Bistable system / reaction coordinate switching.",

        lj:
          "Lennard–Jones potential (centered at origin):\n" +
          "U(r) = 4 ε [ (σ/r)¹² − (σ/r)⁶ ].\n" +
          "Models steric repulsion + van der Waals attraction.",

        morse:
          "Morse binding potential:\n" +
          "U(r) = D_e (1 − exp( −a (r − r₀) ))²  − D_e.\n" +
          "Finite-depth well for ligand–receptor–like interactions."
      };
      
      // little ⓘ tooltip icon
      const potInfo = createElement(
        "span",
        {
          text: " ⓘ",
          style: "margin-left:0.5em; cursor:help; font-size:0.9em;"
        },
        potRow
      );

      // visible text description (separate span, under the row)

      const updatePotTooltip = () => {
        const key = potSelect.value;
        potInfo.setAttribute("title", potHints[key] || "");
      };
      ...
      /*const potDesc = createElement(
        "div",
        {
          style: "margin-left:0.5em; font-size:0.85em; color:#ccc; max-width:800px;"
        },
        controls
      );
      ...
      const updatePotTooltip = () => {
        const key = potSelect.value;
        const hint = potHints[key] || "";
        potInfo.setAttribute("title", hint);   // browser tooltip
        potDesc.textContent = hint;            // always-visible text
      };
      ...
      // --- Global params (with wider ranges) ------------------------------
      ...
      btnStart.addEventListener("click", () => { this.engine.running = true; });
      btnStop.addEventListener("click", () => { this.engine.running = false; });
      btnReset.addEventListener("click", () => { this.engine.reset([0.5, 0.0, 0.0]); });

      // --- Layout buttons (1/2/3 columns) --------------------------------
      ...
      // --- Time-series mode: position vs velocity -------------------------
      ...
      btnPos.addEventListener("click", () => { this.timeSeriesMode = "position"; updateTSButtons(); });
      btnVel.addEventListener("click", () => { this.timeSeriesMode = "velocity"; updateTSButtons(); });
      updateTSButtons();
      ...
      createElement("p", {
        html:
          ...
          "Use layout (1/2/3 columns), time-series (position/velocity) toggle, and the ⓘ icon for physical hints. "
      }, root);

      // --- Visibility logic for parameter rows ----------------------------
      this._updateParamVisibility = () => {
    ...
    _resizeCanvases() {
      ...
      const heights = Array(8).fill(220);/*[
        240,
        240,
        240,
        240,
